<?php
$mapping = require __DIR__ . '/_mapping';
...
function flatSpread($func, $start) {
  return function() {
    ...
    $array = args[start];
        $otherArgs = array_slice($args, 0, $start);

    if ($array) {
      $push.apply($otherArgs, $array);
    }
    if ($start != $lastIndex) {
      $push.apply($otherArgs, array_slice($args, $start + 1));
    }
...
function wrapImmutable($func, $cloner) {
  return function() {
    ...
    $result = args[0] = cloner.apply(undefined, args);
  ...
  $wrappers = {;
    ...
    '$rearg': function($rearg) {
      return function($func, $indexes) {
        $arity = indexes ? indexes.length : 0;
